...
from config.utility_python.reader import File, BinaryFile
...
def create_binary_path(file_path):
...
def launch(blender_file_path, render_directory, render_file_name):
    ...
    blender_src_file = create_binary_path(blender_file_path).get_file_string()
    render_output_file = create_binary_path(render_directory).travel_and_get_file(render_file_name).get_file_string()
